...
export default function Page() {
  const [role, setRole] = useState<"attendee" | "organizer" | "vendor">(
    "attendee",
  );
  ...
  const register = async (event: React.FormEvent) => {
    ...
    setError("");
    if (name.trim().length < 3 || password.length < 8 || !terms) {
      setError(
        "Enter your name, use a password of at least 8 characters, and accept the terms.",
      );
      return;
    }
    setBusy(true);
  ...
  return (
    <AuthShell
      ...
      subtitle="Choose how you want to use Vivnt, then join a community built around real experiences."
      footer={
        <>
      ...
      <form onSubmit={register} className="space-y-5">
        <label className="block text-sm font-semibold">
          ...
          <div className="mt-2 grid grid-cols-3 gap-2">
            {(["attendee", "organizer", "vendor"] as const).map((item) => (
              <button
                key={item}
                type="button"
                onClick={() => setRole(item)}
                className={`rounded-lg border px-2 py-2 text-xs font-semibold capitalize ${role === item ? "border-brown-dark bg-brown-light text-brown-dark" : "border-border"}`}
              >
                {item}
              </button>
            ))}
