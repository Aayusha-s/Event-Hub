...
export async function GET(request: Request) {
  ...
  const [events, organizers, venues, categories, tags, recent, popular] = await Promise.all([
    ...
    session?.user?.id ? SearchHistory.find({ user: new Types.ObjectId(session.user.id) }).sort({ updatedAt: -1 }).limit(5).select("query -_id").lean() : [],
    SearchHistory.aggregate([{ $group: { _id: "$query", count: { $sum: 1 }, latest: { $max: "$updatedAt" } } }, { $sort: { count: -1, latest: -1 } }, { $limit: 5 }]),
  ]);
  return NextResponse.json({ success: true, data: { events, organizers, venues: venues.slice(0, 5), categories: categories.slice(0, 5), tags: tags.slice(0, 8), recent: recent.map(item => item.query), popular: popular.map(item => item._id) } });
}
export async function POST(request: Request) {
  const session = await getServerSession(authOptions); if (!session?.user?.id) return NextResponse.json({ success: true }); const { query } = await request.json(); if (typeof query !== "string" || !query.trim()) return NextResponse.json({ success: true }); await dbConnect(); const normalized = query.trim().slice(0, 100); await SearchHistory.findOneAndUpdate({ user: new Types.ObjectId(session.user.id), query: normalized }, { $set: { query: normalized } }, { upsert: true, new: true, setDefaultsOnInsert: true }); return NextResponse.json({ success: true }, { status: 201 });
}
