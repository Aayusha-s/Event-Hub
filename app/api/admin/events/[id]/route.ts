...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    if (!Types.ObjectId.isValid(id))
      throw new HttpError(400, "Invalid event id.", "INVALID_ID");
    ...
    if (body.approvalStatus !== undefined) {
      ...
      return NextResponse.json({
        success: true,
        data: await updateEventApprovalStatus(event, body.approvalStatus),
      });
    }
    ...
    return NextResponse.json(
      { success: false, error: { message: "Unable to update event status." } },
      { status: 500 },
    );
  }
}
