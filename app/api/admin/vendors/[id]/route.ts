...
export async function PATCH(request: Request, context: RouteContext) {
	try {
		...
		if (!approvalStatus || (approvalStatus !== "approved" && approvalStatus !== "rejected")) {
			throw new HttpError(400, "approvalStatus must be 'approved' or 'rejected'.", "VALIDATION_ERROR");
		}
		...
		return NextResponse.json({ success: false, error: { message: "Unable to update vendor status." } }, { status: 500 });
	}
}
