...
export async function GET(request: Request) {
  try {
    ...
    if (role && !USER_ROLES.includes(role))
      throw new HttpError(400, "Invalid role filter.", "VALIDATION_ERROR");
    if (status && !["active", "suspended"].includes(status))
      throw new HttpError(400, "Invalid status filter.", "VALIDATION_ERROR");
    ...
    return NextResponse.json(
      { success: false, error: { message: "Unable to list users." } },
      { status: 500 },
    );
  }
}
