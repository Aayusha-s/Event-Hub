...
export async function POST(request: Request) {
  try {
    ...
    if (!bookingId || paymentMethod !== "esewa") {
      throw new HttpError(
        400,
        "bookingId and eSewa paymentMethod are required.",
        "VALIDATION_ERROR",
      );
    }

    const result = await initiatePayment(session.user.id, {
      bookingId,
      paymentMethod,
    });
    ...
    return NextResponse.json(
      { success: false, error: { message: "Unable to initiate payment." } },
      { status: 500 },
    );
  }
}
