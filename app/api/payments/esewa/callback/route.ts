...
const logPayment = (stage: string, fields: Record<string, unknown>) => {
	if (process.env.NODE_ENV !== "production") console.info(`[eSewa] ${stage}`, fields);
};
...
const toRecord = (source: URLSearchParams | FormData | Record<string, unknown>) => {
	if (source instanceof URLSearchParams) return Object.fromEntries(source.entries()) as Record<string, unknown>;
	if (source instanceof FormData) return Object.fromEntries(source.entries()) as Record<string, unknown>;
	return source;
};
...
const normalizeCallbackPayload = (sources: Record<string, unknown>[]) => {
	...
	const status = getString(sources, ["status", "payment_status", "transaction_status"]);
	const transactionUuid = getString(sources, ["transaction_uuid", "transactionUuid", "transaction_id", "transactionId", "oid", "pidx"]);
	const transactionCode = getString(sources, ["transaction_code", "transactionCode", "ref_id", "tref", "tid"]);
	const totalAmount = getString(sources, ["total_amount", "amount", "amt"]);
	const productCode = getString(sources, ["product_code", "productCode", "pid"]);
	const signedFieldNames = getString(sources, ["signed_field_names", "signedFieldNames"]);
...
const complete = async (request: Request) => {
	try {
		...
		const dataCandidate = getString(sources, ["data", "payload", "callback", "esewaData"]);
		const decodedData = dataCandidate ? decodeBase64Json(dataCandidate) ?? (() => {
			try {
