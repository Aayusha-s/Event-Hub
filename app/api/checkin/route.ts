...
import { getCheckerDashboard, getRecentCheckIns, listAttendance, processCheckIn, verifyTicket } from "@/services/checkin/checkin.service";
...
export async function GET(request: Request) {
	try {
		...
		if (identifier) return NextResponse.json({ success: true, data: await verifyTicket(identifier) });
		if (params.get("view") === "dashboard") return NextResponse.json({ success: true, data: await getCheckerDashboard(session.user.id) });
		if (params.get("view") === "attendance") return NextResponse.json({ success: true, data: { items: await listAttendance({ search: params.get("search") ?? undefined, eventId: params.get("eventId") ?? undefined, date: params.get("date") ?? undefined }) } });
		const items = await getRecentCheckIns(session.user.id); return NextResponse.json({ success: true, data: { items } });
	} catch (error) {
		...
		return NextResponse.json({ success: false, error: { message: "Unable to fetch recent check-ins." } }, { status: 500 });
	}
}
