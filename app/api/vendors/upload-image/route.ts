...
export async function POST(request: Request) {
    try {
        ...
        if (!(file instanceof File)) {
            throw new HttpError(400, "A vendor image is required.", "VALIDATION_ERROR");
        }
        ...
        const directory = path.join(process.cwd(), "public", "uploads", "vendors");
        ...
        return NextResponse.json(
            {
                success: false,
                error: {
                    message: error instanceof HttpError ? error.message : "Unable to upload vendor image.",
                },
            },
            { status }
        );
