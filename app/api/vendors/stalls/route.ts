...
export async function POST(request: Request) {
  try {
    ...
    const { eventId, stallName, description, stallType, size, bookingFee } =
      body;
    ...
    if (
      ...
      typeof stallType !== "string" ||
      stallType.trim().length < 2 ||
      typeof size !== "string" ||
      !size.trim() ||
      ...
      throw new HttpError(
        400,
        "Please provide valid stall details.",
        "VALIDATION_ERROR",
      );

    const vendor = await bookStall(session.user.id, {
      eventId,
    ...
    return NextResponse.json(
      { success: false, error: { message: "Unable to book stall." } },
      { status: 500 },
    );
  }
}
