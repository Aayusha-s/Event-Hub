...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json(
      { success: false, error: { message: "Unable to fetch vendor profile." } },
      { status: 500 },
    );
  }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    if (!vendor)
      throw new HttpError(404, "Vendor profile not found.", "NOT_FOUND");
    ...
    return NextResponse.json(
      {
        success: false,
        error: { message: "Unable to update vendor profile." },
      },
      { status: 500 },
    );
  }
...
export async function POST(request: Request) {
  try {
    ...
    if (!businessName || !description || !category) {
      throw new HttpError(
        400,
        "businessName, description, and category are required.",
        "VALIDATION_ERROR",
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: { message: "Unable to register vendor." } },
      { status: 500 },
    );
  }
}
