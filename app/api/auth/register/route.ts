...
export async function POST(request: Request) {
	try {
		...
		if (existingUser) {
			return NextResponse.json({ success: false, error: { message: "An account with this email already exists." } }, { status: 409 });
		}

		const password = await bcrypt.hash(input.password, 12);
		const user = await User.create({ ...input, password });
		const response = NextResponse.json(
			{ success: true, data: { id: user._id.toString(), name: user.name, email: user.email, role: user.role } },
			{ status: 201 }
		);
		...
		return NextResponse.json({ success: false, error: { message: "Unable to create account." } }, { status: 500 });
	}
}
