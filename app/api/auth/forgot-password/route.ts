  ...
  try {
    ...
    if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
      return NextResponse.json(
        { success: false, error: { message: "Enter a valid email address." } },
        { status: 400 },
      );
    await dbConnect();
    ...
    if (user) {
      ...
      const emailSent = await sendEmail({
        to: user.email,
        subject: "Reset your Vivnt password",
        text: `Reset your password here: ${resetUrl}`,
        html: `<p>Hello ${user.name},</p><p><a href="${resetUrl}">Reset your Vivnt password</a>. This link expires in one hour.</p>`,
      });
