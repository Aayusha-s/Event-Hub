...
export async function POST(request: Request) {
    try {
        ...
        if (!(file instanceof File)) {
            throw new HttpError(400, "An event image is required.", "VALIDATION_ERROR");
        }
        ...
        if (file.size > 10 * 1024 * 1024) {
            throw new HttpError(400, "Event images must be 10MB or smaller.", "VALIDATION_ERROR");
        }
        ...
        return NextResponse.json(
            {
                success: false,
                error: {
                    message: error instanceof HttpError ? error.message : "Unable to upload event image.",
                },
            },
            { status }
        );
