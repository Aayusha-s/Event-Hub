  ...
  NextResponse.json(
    {
      ...
      error: {
        message:
          e instanceof HttpError ? e.message : "Unable to process reviews.",
      },
    },
...
export async function GET(_: Request, c: C) {
  try {
    const event = await id(c);
    return NextResponse.json({
      success: true,
      data: {
        items: await getReviews(event),
        summary: await getReviewSummary(event),
      },
    });
  } catch (e) {
    return fail(e);
...
export async function POST(r: Request, c: C) {
  try {
    ...
    if (!Number.isInteger(rating) || rating < 1 || rating > 5 || !text)
      throw new HttpError(
        400,
        "A rating from 1 to 5 and review text are required.",
        "VALIDATION_ERROR",
      );
    return NextResponse.json({
      success: true,
      data: await upsertReview(
