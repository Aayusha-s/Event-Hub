...
export async function GET(_request: Request, context: RouteContext) {
  try {
    ...
    const canViewUnapproved =
      session?.user?.id && ["organizer", "admin"].includes(session.user.role)
        ? await findEventForManagement(eventId).then(
            (candidate) =>
              candidate &&
              (session.user.role === "admin" ||
                candidate.organizer.toString() === session.user.id),
          )
        : false;
...
export async function PUT(request: Request, context: RouteContext) {
  try {
    ...
    if (typeof body.action === "string") {
      if (
        session.user.role !== "admin" &&
        ["published", "unpublish"].includes(body.action)
      ) {
        throw new HttpError(
          403,
          "Only an administrator can publish or unpublish an event through approval.",
          "FORBIDDEN",
        );
      }
      if (
        ["draft", "published", "cancelled", "completed"].includes(body.action)
      ) {
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    if (body.action !== "duplicate")
      throw new HttpError(400, "Invalid event action.", "VALIDATION_ERROR");
