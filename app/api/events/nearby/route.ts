...
export async function GET(request: Request) {
	try {
		...
		if (!latStr || !lngStr) {
			throw new HttpError(400, "Latitude (lat) and longitude (lng) are required query parameters.", "VALIDATION_ERROR");
		}
		...
		if (isNaN(lat) || lat < -90 || lat > 90 || isNaN(lng) || lng < -180 || lng > 180) {
			throw new HttpError(400, "Invalid latitude or longitude coordinates.", "VALIDATION_ERROR");
		}
		...
		return NextResponse.json({ success: false, error: { message: "Unable to fetch nearby events." } }, { status: 500 });
	}
}
