...
const errorResponse = (error: unknown) => {
  ...
  return NextResponse.json(
    { success: false, error: { message: "Unable to process event request." } },
    { status: 500 },
  );
};
...
export async function GET(request: Request) {
  try {
    ...
    const dateFrom = parseDate(
      searchParams.get("dateFrom") ?? searchParams.get("date"),
      "dateFrom",
    );
    const dateTo = parseDate(
      searchParams.get("dateTo") ?? searchParams.get("date"),
      "dateTo",
      true,
    );

    if (organizer && !Types.ObjectId.isValid(organizer))
      throw new HttpError(
        400,
        "organizer must be a valid id.",
        "VALIDATION_ERROR",
      );
    ...
    if (status && !eventStatuses.includes(status as EventStatus))
    ...
    const location =
      ...
      searchParams.get("filterLocation")?.trim();
    const parsePrice = (value: string | null, field: string) => {
      if (!value) return undefined;
      const parsed = Number(value);
    ...
    const priceMin = parsePrice(searchParams.get("priceMin"), "priceMin");
    const priceMax = parsePrice(searchParams.get("priceMax"), "priceMax");
    const rawSort = searchParams.get("sort") ?? searchParams.get("popularity");
    ...
    const sort = (sortMap[rawSort ?? ""] ?? rawSort) as
      | "newest"
      | "oldest"
      | "trending"
      | "popular"
      ...
      | "priceDesc"
      | null;
    const rating = parsePrice(searchParams.get("rating"), "rating");
    ...
    const free = booleanParam("free");
    const online = booleanParam("online");
    const parseCoordinate = (name: "lat" | "lng") => {
      const value = searchParams.get(name);
      if (!value) return undefined;
    ...
    const latitude = parseCoordinate("lat"),
      ...
      distanceKm = parsePrice(searchParams.get("distanceKm"), "distanceKm");
    ...
    if (priceMin !== undefined && priceMax !== undefined && priceMax < priceMin)
      throw new HttpError(
        400,
        "priceMax must be greater than priceMin.",
        "VALIDATION_ERROR",
      );
    if (
      sort &&
      ...
      throw new HttpError(400, "sort is invalid.", "VALIDATION_ERROR");
    if (
      availability &&
      availability !== "available" &&
      availability !== "soldOut"
    )
      throw new HttpError(400, "availability is invalid.", "VALIDATION_ERROR");
    ...
    if (
      (timeFrom && !/^\d{2}:\d{2}$/.test(timeFrom)) ||
      (timeTo && !/^\d{2}:\d{2}$/.test(timeTo))
    )
      throw new HttpError(
        400,
        "time filters must use HH:MM.",
        "VALIDATION_ERROR",
      );
