...
import { cancelTicket, getTicketById, updateTicketStatus } from "@/services/tickets/ticket.service";
...
export async function GET(_request: Request, context: RouteContext) {
	try {
		...
		const ticket = await getTicketById(new Types.ObjectId(id), new Types.ObjectId(session.user.id), session.user.role);
		...
		return NextResponse.json({ success: false, error: { message: "Unable to fetch ticket details." } }, { status: 500 });
	}
}
...
export async function PATCH(request: Request, context: RouteContext) {
	try {
		...
		if (!ticketStatus || (ticketStatus !== "active" && ticketStatus !== "cancelled")) {
			throw new HttpError(400, "ticketStatus must be 'active' or 'cancelled'.", "VALIDATION_ERROR");
		}
...
export async function DELETE(_request: Request, context: RouteContext) {
	try {
		...
		const { id } = await context.params;
		if (!Types.ObjectId.isValid(id)) throw new HttpError(400, "Ticket id is invalid.", "INVALID_ID");
		const ticket = await cancelTicket(new Types.ObjectId(id), new Types.ObjectId(session.user.id));
		return NextResponse.json({ success: true, data: ticket });
	} catch (error) {
		if (error instanceof HttpError) {
			return NextResponse.json({ success: false, error: { message: error.message, code: error.code } }, { status: error.statusCode });
		...
		return NextResponse.json({ success: false, error: { message: "Unable to cancel ticket." } }, { status: 500 });
	}
}
