...
export async function GET() {
  ...
  const [activityDocs, photoDocs, eventResult, topicDocs, memberDocs] =
    await Promise.all([
      Activity.find()
        .populate("user", "name profileImage role username")
        .sort({ createdAt: -1 })
        .limit(8)
        .lean(),
      Photo.find()
  ...
  const feed = activityDocs.map((activity: any) => ({
    ...
    profileUrl: activity.user?.username
      ? `/userprofile?username=${encodeURIComponent(activity.user.username)}`
      : `/userprofile?userId=${activity.user?._id ?? ""}`,
    likes: 0,
    comments: 0,
    shares: 0,
  }));
  ...
  const trendingTopics = topicDocs.map((topic: any) => ({
    title: topic._id,
  ...
  const members = memberDocs.map((member: any) => ({
    ...
    profileUrl: member.username
      ? `/userprofile?username=${encodeURIComponent(member.username)}`
      : `/userprofile?userId=${member._id}`,
  }));

  return NextResponse.json({
    success: true,
    data: { feed, photos, events, trendingTopics, meetups, members },
  });
}
