...
type EditableBasicInformationDraft = BasicInformationDraft & {
  images?: string[];
};
...
const Page = () => {
      ...
      fetch(`/api/events/${id}`)
        ...
        .then((r) => {
          ...
          saveDraft("eventDetails", {
            ...
            city: "",
            state: "",
            eventCapacity: String(event.capacity),
            allowVendorStalls: Boolean(event.allowVendorStalls),
            stallOpeningDate: event.stallOpeningDate
              ? new Date(event.stallOpeningDate).toISOString().slice(0, 10)
              : "",
            stallApplicationDeadline: event.stallApplicationDeadline
              ? new Date(event.stallApplicationDeadline)
                  .toISOString()
                  .slice(0, 10)
              : "",
            stallCapacity: event.stallCapacity
              ? String(event.stallCapacity)
              : "",
            stallCategories: (event.stallCategories ?? []).join(", "),
          });
          saveDraft("eventInfo", {
            tickets: event.ticketTypes.map(
              (ticket: {
                name: string;
                quantity: number;
                price: number;
  ...
  const handleNext = () => {
    ...
    router.push(
      `/create-event/step-2${eventId() ? `?eventId=${eventId()}` : ""}`,
    );
  };
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        {images.length > 0 && (
          <div className="mt-4">
            <h3 className="mb-3 text-sm font-medium text-text-dark">
