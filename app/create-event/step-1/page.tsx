...
const Page = () => {
	...
	useEffect(() => { const id = eventId(); if (!id) return; fetch(`/api/events/${id}`).then(r => r.json()).then(r => { if (!r.success) return; const event = r.data; const start = new Date(event.startDate), end = new Date(event.endDate); saveDraft('basicInformation', { title: event.title, category: event.category, description: event.description, images: event.images ?? [], eventId: id }); saveDraft('eventDetails', { startDate: start.toISOString().slice(0, 10), endDate: end.toISOString().slice(0, 10), startTime: start.toISOString().slice(11, 16), endTime: end.toISOString().slice(11, 16), venueName: event.venue, streetAddress: '', city: '', state: '', eventCapacity: String(event.capacity), allowVendorStalls: Boolean(event.allowVendorStalls), stallOpeningDate: event.stallOpeningDate ? new Date(event.stallOpeningDate).toISOString().slice(0, 10) : '', stallApplicationDeadline: event.stallApplicationDeadline ? new Date(event.stallApplicationDeadline).toISOString().slice(0, 10) : '', stallCapacity: event.stallCapacity ? String(event.stallCapacity) : '', stallCategories: (event.stallCategories ?? []).join(', ') }); saveDraft('eventInfo', { tickets: event.ticketTypes.map((ticket: { name: string; quantity: number; price: number; description?: string }) => ({ ticketName: ticket.name, quantity: String(ticket.quantity), price: String(ticket.price), description: ticket.description ?? '' })) }); setTitle(event.title); setCategory(event.category); setDescription(event.description); }); }, []);
