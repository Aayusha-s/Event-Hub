...
const Page = () => {
  ...
  const handleFreeToggle = (nextValue: boolean) => {
    ...
    setTickets((prev) =>
      prev.map((ticket) => ({ ...ticket, price: nextValue ? "0" : "" })),
    );
    saveEventInfoTickets(
      tickets.map((ticket) => ({ ...ticket, price: nextValue ? "0" : "" })),
      nextValue,
    );
  };
  ...
  const handleNext = () => {
    ...
    router.push(
      `/create-event/step-4${eventId() ? `?eventId=${eventId()}` : ""}`,
    );
  };
        ...
        {tickets.map((ticket, index) => (
          <div key={index} className="surface-card p-4">
            <div className="mb-4 flex items-center justify-between">
              ...
              {tickets.length > 1 && (
                <CircleX
                  className="cursor-pointer text-text-light transition-colors hover:text-primary"
            ...
            <div className="grid grid-cols-1 gap-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateTicket(index, "ticketName", e.target.value)
                  }
                >
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={
                    isFreeEvent
                      ? "Free event automatically sets price to 0"
                      : "e.g. 50.00"
                  }
