...
const Page = () => {
  ...
  const handleNext = () => {
    if (
      ...
      !eventCapacity ||
      (allowVendorStalls &&
        (!stallOpeningDate || !stallApplicationDeadline || !stallCapacity))
    ) {
    ...
    router.push(
      `/create-event/step-3${eventId() ? `?eventId=${eventId()}` : ""}`,
    );
  };
  ...
  const handlePrevious = () => {
    saveDraft("eventDetails", buildEventDetails());
    router.push(
      `/create-event/step-1${eventId() ? `?eventId=${eventId()}` : ""}`,
    );
  };
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-text-light">
              Approved vendors can apply only after this event is approved.
            </p>
          </div>
          <div className="flex gap-2">
            <Button
              ...
              text="Yes"
              size="sm"
              variant={allowVendorStalls ? "cta" : "secondary"}
              onClick={() => setAllowVendorStalls(true)}
            />
            <Button
              type="button"
              text="No"
              size="sm"
              variant={!allowVendorStalls ? "cta" : "secondary"}
              onClick={() => setAllowVendorStalls(false)}
            />
          </div>
        </div>
        {allowVendorStalls && (
          <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <label className="text-sm font-medium">
              ...
              <input
                type="date"
                className="mt-1 w-full rounded-xl border border-border bg-surface px-3 py-3 focus-ring"
                value={stallOpeningDate}
                onChange={(e) => setStallOpeningDate(e.target.value)}
                required
              />
            </label>
            <label className="text-sm font-medium">
              ...
              <input
                type="date"
                className="mt-1 w-full rounded-xl border border-border bg-surface px-3 py-3 focus-ring"
                value={stallApplicationDeadline}
                onChange={(e) => setStallApplicationDeadline(e.target.value)}
                required
              />
            </label>
            <label className="text-sm font-medium">
            ...
            <label className="text-sm font-medium">
              ...
              <input
                ...
                value={stallCategories}
                onChange={(e) => setStallCategories(e.target.value)}
              />
            </label>
          </div>
        )}
