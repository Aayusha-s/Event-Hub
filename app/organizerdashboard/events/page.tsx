    ...
    fetch("/api/organizer/dashboard", { cache: "no-store" })
      .then(async (response) => {
        ...
        if (!response.ok || !result.success)
          throw new Error(
            result.error?.message || "Unable to load your events.",
          );
  ...
  return (
    <section className="app-page font-cause text-text-dark">
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        {["all", "upcoming", "past", "archived"].map((value) => (
          <button
            key={value}
            type="button"
            onClick={() => setFilter(value)}
            className={`rounded-lg border px-3 py-2 text-sm capitalize ${filter === value ? "border-brown-dark bg-brown-light" : "border-border"}`}
          >
            {value}
          </button>
        ))}
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {events.filter(
          (event) =>
            filter === "all" ||
            (filter === "archived" && event.status === "completed") ||
            (filter === "past" &&
          ...
          <p className="text-text-light">No events in this view yet.</p>
        )}
      </div>
    </section>
  );
