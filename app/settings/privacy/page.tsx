...
const rows = [
  ...
  },
  {
    ...
    description: "Allow others to see events you are attending.",
  },
  {
    key: "allowMessages",
    title: "Allow direct messages",
    description: "Let other users message you through Vivnt.",
  },
];
    ...
    fetch("/api/settings", { cache: "no-store" })
      .then(async (response) => {
        ...
        if (!response.ok || !result.success)
          throw new Error(
            result.error?.message ?? "Unable to load privacy preferences.",
          );
  ...
  const save = async () => {
    ...
    try {
      const response = await fetch("/api/settings", {
        ...
        body: JSON.stringify({ privacy: preferences }),
      });
      const result = await response.json();
      if (!response.ok || !result.success)
        throw new Error(
          result.error?.message ?? "Unable to save privacy preferences.",
        );
      ...
      <div className="flex flex-col gap-6 lg:flex-row">
        ...
        <div className="surface-card min-w-0 flex-1 p-5 sm:p-7">
          ...
          <p className="mt-1 text-sm text-text-light">
            Control how your account information is shared.
          </p>
          {message && (
          ...
          <div className="mt-5 divide-y divide-divider">
            {rows.map((item) => (
              <div
                key={item.key}
                className="flex min-w-0 items-center justify-between gap-5 py-5"
              >
