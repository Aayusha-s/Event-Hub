...
const categoryPalette = [
  ...
  },
  {
    ...
    icon: <PaintBucket className="text-yellow-500" />,
  },
  {
...
const Page = () => {
  const [activeTab, setActiveTab] = useState<
    "feed" | "photos" | "events" | "trending" | "meetups" | "members"
  >("feed");
    ...
    fetch("/api/community", { cache: "no-store" })
      .then(async (response) => {
        ...
        if (!response.ok || !result.success)
          throw new Error(result.error?.message || "Unable to load community.");
  ...
  if (error || !data) {
    return (
      <section className="flex flex-col my-4 mx-2 px-4 font-cause text-text-dark md:my-3 md:mx-3 md:px-3 lg:my-4 lg:mx-4 lg:px-4 xl:my-6 xl:mx-6 xl:px-6 2xl:my-8 2xl:mx-8 2xl:px-8">
        <h1 className="font-dynapuff text-2xl font-bold">Community</h1>
        <p className="mt-2 text-red-600">
          {error || "Unable to load community."}
        </p>
      </section>
    );
  }
  ...
  return (
    <section
      className="flex flex-col my-4 mx-2 px-4 font-cause text-text-dark 
      ...
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div className="space-y-2">
          ...
          <p className="text-base md:text-lg max-w-2xl">
            Connect with event lovers, share experiences, and discover new
            opportunities
          </p>
        </div>
          ...
          <div className="space-y-8">
            ...
            <div>
              <div className="flex flex-row items-center justify-between mb-4">
                <h2 className="font-dynapuff text-xl md:text-2xl font-semibold">
                  Latest Photos
                </h2>
                <Button
                  text="View Photos"
        ...
        {activeTab === "photos" && (
          <div>
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-dynapuff text-lg md:text-xl font-semibold">
                Photo Gallery
              </h3>
              <Button
                text="Back to Feed"
            ...
            {data.trendingTopics.map((topic, index) => {
              ...
              return (
                <TrendingTopics
                  ...
                  iconBoxColor={palette.iconBoxColor}
                  icon={palette.icon}
                  title={topic.title}
        ...
        {activeTab === "meetups" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-2 2xl:grid-cols-3 gap-6">
            {data.meetups.map((meetup) => (
              <MeetUp key={`${meetup.title}-${meetup.date}`} {...meetup} />
            ))}
          </div>
        )}
        ...
        {activeTab === "members" && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {data.members.map((member) => (
              <FeaturedMember key={member.name} {...member} />
            ))}
          </div>
        )}
      </div>
