...
export default function AttendancePage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const params = new URLSearchParams({ view: "attendance" });
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-10 font-cause text-text-dark">
      <h1 className="text-3xl font-dynapuff font-semibold">Attendance Log</h1>
      <div className="my-6 grid gap-3 md:grid-cols-3">
        <input
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          placeholder="Ticket number or type"
        ...
        <div className="overflow-x-auto rounded-xl border border-border bg-surface">
          <table className="w-full min-w-[780px] text-left">
            ...
            <tbody>
              {items.length === 0 ? (
                <tr>
                  <td colSpan={6} className="p-5 text-text-light">
                ...
                items.map((item) => (
                  ...
                  >
                    ...
                    <td className="p-3">{item.event?.title ?? "-"}</td>
                    ...
                    <td className="p-3">{item.checkedInBy?.name ?? "-"}</td>
                  </tr>
                ))
              )}
            </tbody>
