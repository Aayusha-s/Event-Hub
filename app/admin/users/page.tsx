...
export default function AdminUsersPage() {
  ...
  const updateUser = async (id: string, payload: Record<string, string>) => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok || !json.success)
        throw new Error(json.error?.message ?? "Unable to update user.");
      setUsers((current) =>
        current.map((user) => (user._id === id ? json.data : user)),
      );
  ...
  const deactivate = async (user: AdminUser) => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok || !json.success)
        throw new Error(json.error?.message ?? "Unable to deactivate user.");
      setUsers((current) =>
        current.map((item) => (item._id === user._id ? json.data.user : item)),
      );
      setMessage("User deactivated.");
    } catch (cause) {
      setError(
        cause instanceof Error ? cause.message : "Unable to deactivate user.",
      );
    } finally {
      setSavingId("");
    }
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-12 font-cause text-text-dark">
      ...
      <p className="mb-6 text-text-light">
        Search, manage account access, and assign validated application roles.
      </p>
      <div className="mb-5 grid gap-3 rounded-xl border border-border bg-surface p-4 md:grid-cols-4">
        <input
          value={search}
                  ...
                  >
                    ...
                    <td className="p-3">
                      <select
                        ...
                        onChange={(e) =>
                          void updateUser(user._id, { role: e.target.value })
                        }
                    ...
                    <td className="p-3">
                      <button
                        ...
                        onClick={() =>
                          user.status === "active"
                            ? void deactivate(user)
                            : void updateUser(user._id, { status: "active" })
                        }
