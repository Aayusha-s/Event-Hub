...
const CATEGORY_CONFIG: Record<
    string,
    { label: string; icon: React.ReactNode; color: string; gradient: string; description: string }
> = {
    music: {
...
export default function CategoriesPage() {
    ...
    const selectedCategoryConfig = selectedCategory
        ? CATEGORY_CONFIG[selectedCategory.toLowerCase()] ||
          { label: selectedCategory, icon: <Zap />, color: 'text-gray-600', gradient: 'from-gray-500 to-gray-600' }
        : null
    ...
    return (
        <div className='min-h-screen bg-gradient-to-b from-background to-white'>
                ...
                <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
                    <div className='text-center max-w-3xl mx-auto'>
                        ...
                        <p className='text-lg text-text-light leading-relaxed'>
                            Explore events across diverse categories and discover what excites you. From music and arts to technology and wellness, find your next unforgettable experience.
                        </p>
                    </div>
                </div>
            </section>
            ...
            <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12'>
                ...
                {selectedCategory && (
                    <div className={`mb-10 p-6 rounded-xl border border-border bg-gradient-to-r ${selectedCategoryConfig?.gradient} text-white`}>
                        <div className='flex items-center justify-between'>
                            <div>
                                <div className='flex items-center gap-3 mb-2'>
                                    {selectedCategoryConfig?.icon}
                                    <h2 className='text-2xl font-bold'>{selectedCategoryConfig?.label}</h2>
                                </div>
                                <p className='text-white/90'>{selectedCategoryConfig?.description}</p>
                                <p className='text-sm text-white/75 mt-2'>{filteredEvents.length} events available</p>
                            </div>
                            <button
                    ...
                    <>
                        {filteredEvents.length > 0 ? (
                            <div>
                                <div className='mb-8'>
                                    ...
                                    <p className='text-text-light'>
                                        {filteredEvents.length} events found {selectedCategory && `in ${selectedCategoryConfig?.label}`}
                                    </p>
                                </div>
                                ...
                                {filteredEvents.length > 12 && (
                                    <div className='mt-10 text-center'>
                                        <Link href={`/explore-events${selectedCategory ? `?category=${selectedCategory}` : ''}`}>
                            ...
                            <div className='text-center py-20'>
                                ...
                                <h3 className='text-2xl font-bold text-text-dark mb-3'>No events found</h3>
                                <p className='text-text-light mb-8 max-w-md mx-auto'>
                                    There are no events in this category yet. Check back soon or explore other categories!
                                </p>
                                <button
                                    onClick={() => setSelectedCategory(null)}
            ...
            <section className='bg-gradient-to-r from-brown-normal to-brown-dark py-16 md:py-20 text-white mt-20'>
                <div className='max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center'>
                    ...
                    <p className='text-lg text-brown-light mb-8 max-w-2xl mx-auto'>
                        Create your own event and bring your community together. Host memorable experiences on Vivnt and reach event enthusiasts.
                    </p>
