...
function TagsPage() {
  ...
  useEffect(() => {
    fetch("/api/events/taxonomy")
      .then((r) => r.json())
      .then((r) => {
        if (r.success) setTags(r.data.tags);
      });
    const q = new URLSearchParams({ status: "published", pageSize: "24" });
    if (tag) q.set("tags", tag);
    fetch(`/api/events?${q}`)
      .then((r) => r.json())
      .then((r) => {
        if (r.success) setEvents(r.data.items);
      });
  }, [tag]);
  return (
      ...
      <div className="bg-brown-light w-full rounded-xl mb-4 p-4 space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          {tags.map((item) => (
            <a
              key={item.name}
              href={`/event-tags?tag=${encodeURIComponent(item.name)}`}
            >
              <Button text={`${item.name} (${item.count})`} variant="tag" />
            </a>
          ))}
        </div>
      </div>
      {tag && <p className="mb-4">Active tag: {tag}</p>}
      <h3 className="font-bold mb-4">All Events</h3>
